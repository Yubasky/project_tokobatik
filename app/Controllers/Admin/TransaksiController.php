...
class TransaksiController extends BaseController
{
    ...
    public function store()
    {
        ...
        foreach ($produkIds as $idx => $id_produk) {
            ...
            if ($produk['stok'] < $qty) {
                $db->transRollback();
                return redirect()->back()->withInput()->with('error', "Stok produk '{$produk['nama_produk']}' tidak mencukupi. Tersedia: {$produk['stok']}");
            }
    ...
    public function show(int $id)
    {
        ...
        if (!$transaksi) return redirect()->to('/admin/transaksi')->with('error', 'Transaksi tidak ditemukan.');
    ...
    public function delete(int $id)
    {
        ...
        $details = $this->detailModel->where('id_transaksi', $id)->findAll();
        foreach ($details as $d) {
            $produk = $this->produkModel->find($d['id_produk']);
            if ($produk) {
                $this->produkModel->update($d['id_produk'], ['stok' => $produk['stok'] + $d['jumlah']]);
            }
        }

        $this->model->delete($id);

        // Hapus pemasukan & kas yang terkait jika ada
        $pemasukanModel = new \App\Models\PemasukanModel();
        $pemasukan = $pemasukanModel->where('keterangan', 'Penjualan Transaksi #' . $id)->first();
        if ($pemasukan) {
            $pemasukanModel->deleteWithKas($pemasukan['id_pemasukan']);
        }

        return redirect()->to('/admin/transaksi')->with('success', 'Transaksi berhasil dihapus dan stok dikembalikan.');
    }
}
